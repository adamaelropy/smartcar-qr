...
async function getMyVehicleQr(req, res) {
    try {
        ...
        return res.status(500).json({ success: false, message: "Failed to retrieve vehicle QR information." });
    }
}
...
async function getVehicleQrById(req, res) {
    ...
    try {
        const vehicle = await prisma.vehicle.findFirst({
            where: { vehicle_id: numericVehicleId, user_id: req.user.userId },
            select: vehicleQrSelect,
        });
        if (!vehicle) return res.status(404).json({ success: false, message: "Vehicle not found." });
        return res.json({ success: true, vehicle });
...
module.exports = { getMyVehicleQr, getVehicleQrById };
