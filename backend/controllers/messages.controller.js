...
const { createBigIntId, normalizeConversationPair } = require("../utils/ids");
const { classifyMessageText, formatTime } = require("../utils/messageClassify");
...
async function createConversationMessage({ conversationId, senderId, recipientId, body, kind = "TEXT" }) {
    ...
    await prisma.$transaction([
        prisma.conversationMessage.create({
            data: { message_id: messageId, conversation_id: conversationId, sender_id: senderId, recipient_id: recipientId, body, kind },
        }),
...
async function getOrCreateConversation(userId, otherUserId) {
    ...
    let conversation = await prisma.conversation.findFirst({
        where: { participant_a_id: participantAId, participant_b_id: participantBId },
        select: { conversation_id: true },
    });
    if (!conversation) {
        conversation = await prisma.conversation.create({
            data: { conversation_id: createBigIntId(), participant_a_id: participantAId, participant_b_id: participantBId },
            select: { conversation_id: true },
        });
    }
    return conversation;
...
function buildConversationThread(conversation, userId) {
    const counterpart = conversation.participant_a_id === userId ? conversation.participantB : conversation.participantA;
    const lastMessage = conversation.messages[conversation.messages.length - 1] || null;
    const unread = conversation.messages.filter((m) => m.recipient_id === userId && !m.read_at).length;
    return {
        ...
        unread,
        blocked: conversation.messages.some((m) => classifyMessageText(m.body) === "blocked"),
        emergency: conversation.messages.some((m) => m.kind === "EMERGENCY" || classifyMessageText(m.body) === "emergency"),
        latestIncomingText: [...conversation.messages].reverse().find((m) => m.recipient_id === userId)?.body || null,
...
function buildAnonymousThreads(communications) {
    ...
    const threads = [];
    for (const comms of grouped.values()) {
        ...
        threads.push({
            ...
            unread: comms.filter((c) => !c.read).length,
...
async function getMessages(req, res) {
    ...
    try {
        const conversations = await prisma.conversation.findMany({
            where: { OR: [{ participant_a_id: userId }, { participant_b_id: userId }] },
            orderBy: { last_message_at: "desc" },
            select: {
                conversation_id: true,
                participant_a_id: true,
                participant_b_id: true,
                participantA: { select: { user_id: true, username: true, name: true } },
                participantB: { select: { user_id: true, username: true, name: true } },
                messages: {
                    orderBy: { created_at: "asc" },
                    select: { message_id: true, sender_id: true, recipient_id: true, body: true, kind: true, read_at: true, created_at: true },
                },
            },
        });

        const threads = conversations.map((c) => buildConversationThread(c, userId));

        const ownedVehicles = await prisma.vehicle.findMany({ where: { user_id: userId }, select: { vehicle_id: true } });
        ...
        if (ownedVehicleIds.length > 0) {
            const anonCommunications = await prisma.communication.findMany({
                where: { vehicle_id: { in: ownedVehicleIds }, claimed_by_user_id: null, conversation_id: null },
                orderBy: { created_at: "asc" },
                select: { communication_id: true, vehicle_id: true, type: true, message: true, read: true, created_at: true, source: true },
            });
            anonThreads = buildAnonymousThreads(anonCommunications);
        }
        ...
        const finalThreads = allThreads.map(({ _sortKey, vehicleId, anonHash, ...rest }) => ({
            ...rest,
            messages: rest.messages.map(({ created_at, ...m }) => m),
        }));
...
async function markThreadRead(req, res) {
    ...
    try {
        if (String(threadId).startsWith("anon-vehicle-")) {
            ...
            if (!match) {
                ...
                if (legacyMatch) {
                    ...
                    if (!vehicleLegacy) return res.status(404).json({ success: false, message: "Conversation not found." });
            ...
            if (!vehicle) return res.status(404).json({ success: false, message: "Conversation not found." });
        ...
        const conversation = await prisma.conversation.findFirst({
            where: { conversation_id: conversationId, OR: [{ participant_a_id: req.user.userId }, { participant_b_id: req.user.userId }] },
            select: { conversation_id: true },
        });
        if (!conversation) return res.status(404).json({ success: false, message: "Conversation not found." });

        const result = await prisma.conversationMessage.updateMany({
            where: { conversation_id: conversationId, recipient_id: req.user.userId, read_at: null },
...
async function sendMessage(req, res) {
    ...
    try {
        ...
        const conversation = await prisma.conversation.findFirst({
            where: { conversation_id: conversationId, OR: [{ participant_a_id: req.user.userId }, { participant_b_id: req.user.userId }] },
            select: { conversation_id: true, participant_a_id: true, participant_b_id: true },
        });
        if (!conversation) return res.status(404).json({ success: false, message: "Conversation not found." });

        const recipientId = conversation.participant_a_id === req.user.userId ? conversation.participant_b_id : conversation.participant_a_id;
        const kind = mode === "emergency" || classifyMessageText(body) === "emergency" ? "EMERGENCY" : "TEXT";

        const messageId = await createConversationMessage({ conversationId, senderId: req.user.userId, recipientId, body, kind });

        return res.json({
            success: true,
            threadId,
            message: { id: String(messageId), sender: "me", text: body, time: formatTime(new Date()), read: false, kind },
        });
    } catch (error) {
...
module.exports = { createConversationMessage, getOrCreateConversation, getMessages, markThreadRead, sendMessage };
